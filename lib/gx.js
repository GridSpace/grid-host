...
class GXName {
    constructor(host, name, callback) {
        const ctrl = new FFControl(host, 8899, error => {
            if (callback) {
                callback({error});
            } else {
                console.error(error);
        ...
        ctrl.sendCommand("M601 S1", lines => { console.log({m601: lines}) });
        ctrl.sendCommand("M610 "+name, lines => { console.log({m610: lines}) });
        ctrl.sendCommand("M602", lines => {
        ...
        // control release
        ctrl.sendCommand("M602", lines => { console.log({m602:lines}) });
    }
}
...
class GXStatus {
    constructor(host, port, callback) {
        ...
        ctrl.sendCommand("M115", lines => { this.status.info = lines });
        ctrl.sendCommand("M119", lines => { this.status.status = lines });
        ctrl.sendCommand("M27", lines => {
...
if (!module.parent) {
    ...
    switch (cmd) {
        ...
        case 'scan':
            ...
            break;
        case 'name':
            new GXName(arg.shift(), arg.shift(), () => {
                console.log({set_name: true});
                process.exit();
            });
            break;
        default:
